...
test('table video is opt-in, seat-authorized, idempotent, scoped, and independent from the hand',async()=>{
 ...
 try{
  ...
  await req(ta,{visitor:{id:randomUUID(),name:'Pessoa',position:{x:TABLE.x,z:TABLE.z+1.3},frozen:true},callActive:true});assert.equal(row.payload.poker.players[0].left,undefined,'Opening other UI must not leave poker');
