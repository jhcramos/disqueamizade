...
const now=Date.now(),v=(name,position={x:-5,z:-5.2})=>({id:randomUUID(),name,position,available:true});
...
test('quick any preference accepts music and cross-room options without overriding a refusal',()=>{
 const s=createLife(),a=v('Ana'),b=v('Bruno',{x:-5,z:2.8}),people=[a,b];ask(s,a,people,'Topo qualquer coisa');ask(s,b,people,'Ouvir música juntos');
...
test('bar requires both visitors to have confirmed adult access',()=>{
 const s=createLife(),a=v('Ana'),b=v('Bruno',{x:5,z:2.8}),people=[a,b];ask(s,a,people,'Topo qualquer coisa');ask(s,b,people,'Topo qualquer coisa');assert.equal(companyCandidates(s,a,people,now).length,0);
...
test('cross-room accepted reservation survives travel but expires without arrival',()=>{
 const s=createLife(),a=v('Ana'),b=v('Bruno',{x:-5,z:2.8}),people=[a,b];ask(s,a,people,'Topo qualquer coisa');ask(s,b,people,'Topo qualquer coisa');const r=concierge(s).requests[a.id],d=preparedDecision(r.text,companyCandidates(s,a,people,now));applyCompanyDecision(s,a.id,r.id,d,people,now);const p=concierge(s).proposals[0];assert.ok(p);
