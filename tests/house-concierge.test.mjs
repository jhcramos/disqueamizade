...
const now=Date.now();const visitor=name=>({id:randomUUID(),name,position:{...areaById('garage').arrival},available:true});
...
test('garagem, sala and bar have reachable seats; poker table is never repurposed',()=>{
 for(const room of ['garage','living','bar']){
  const {s,a,b,people}=setup();a.position=b.position={...areaById(room==='bar'?'dining':room).arrival};
