...
async function enter(){
 ...
 await context.route('**/api/house-residents',async route=>{const body=route.request().postDataJSON(),state=createLife();state.residents[0].position={x:body.visitor.position.x+distance,z:body.visitor.position.z};state.residents[0].path=[];state.residents[1].position={x:8,z:3};state.residents[1].path=[];state.social.solo[body.visitor.id]=solo;if(body.command)calls.push(body.command);
...
if(process.env.CHECK!=='gestures'){
 const t=await enter();try{
  ...
  await t.p.getByRole('button',{name:'Interagir com Dora',exact:true}).click();await t.p.getByRole('textbox',{name:'O que você tem vontade de fazer?'}).waitFor();assert.equal(t.calls.length,0,'Opening options is not consent');assert.deepEqual(t.errors,[]);console.log('PASS nearby host greeting, dismissal, explicit options and no automatic AI');
