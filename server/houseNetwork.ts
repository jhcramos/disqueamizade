...
export type NetworkState={seq:number;members:Record<string,Member>;packets:Packet[];departed?:Record<string,number>};
...
export function exchangeNetwork(s:NetworkState,owner:string,body:any,now:number){
 ...
 if(body.depart===true){delete s.members[body.id];s.departed[session]=now+90000;return {cursor:s.seq,packets:[],roster:[]};}
 ...
 return {cursor:s.seq,packets,roster};
}
