...
## Estado e limites
Camada de simulação independente da renderização. Sincronizar estado entre abas da visita local por coordenador; registrar explicitamente o limite local. Não anunciar sincronização entre dispositivos enquanto não houver serviço autoritativo implantado. Movimentos e rotinas preparados não exigem LLM. Provedor escolhido pelo usuário: DeepInfra, modelo zai-org/GLM-5.3-Flash, por uso. Banco configurado em uquztttljpswheiikbkw. Chave substituída e validada com HTTP 200 na DeepInfra; armazenada como variável sensível da Vercel Production; não simular integração com IA generativa nem expor chaves no navegador.
