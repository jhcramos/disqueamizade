...
export function PokerPanel({open,onClose,getVisitor,onSeat}:{open:boolean;onClose:()=>void;getVisitor:()=>Visitor;onSeat:(seat:number|null)=>void}){
 ...
 useEffect(()=>{
  ...
  try{token=sessionStorage.getItem('house-poker-session')||'';}catch{/* private mode */}
  async function poll(){if(closed||fetching||(!props.current.open&&!joined&&!pending))return;fetching=true;
   const sent=pending;
   try{
    const response=await fetch('/api/house-residents',{method:'POST',headers:{'Content-Type':'application/json',...(token?{'X-Resident-Session':token}:{})},body:JSON.stringify({feature:'poker',visitor:props.current.getVisitor(),command:sent}),signal:AbortSignal.timeout(10000)});
    if(closed)return;
    if(response.status===401){token='';pending=undefined;setBusy(false);try{sessionStorage.removeItem('house-poker-session');}catch{}return;}
    ...
    if(data.identity?.token){token=data.identity.token;try{sessionStorage.setItem('house-poker-session',token);}catch{}}
    const me=data.game.players.find((p:{id:string;seat:number})=>p.id===data.game.me);
    if(me&&!joined)props.current.onSeat(me.seat);
    if(!me&&joined)props.current.onSeat(null);
    joined=!!me;setGame(data.game);setOnline(true);
 ...
 if(!open)return null;
 const me=game?.players.find(p=>p.id===game.me),near=Math.hypot(getVisitor().position.x-TABLE.x,getVisitor().position.z-TABLE.z)<2.1;
 const action=(a:PokerAction)=>command.current?.(a);
 const phase={waiting:'A mesa está aberta',preflop:'Pré-flop',flop:'Flop',turn:'Turn',river:'River',showdown:'Fim da mão'};
 return <section className="house-poker" aria-label="Mesa de pôquer" onPointerDown={e=>e.stopPropagation()} onKeyDown={e=>{if(e.key==='Escape'){e.stopPropagation();onClose();}}}>
