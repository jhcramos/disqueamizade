...
export function companyCandidates(s:LifeState,a:Visitor,visitors:Visitor[],now:number):Candidate[]{
 const c=concierge(s);if(!requested(s,a,now)||busy(s,a.id)||c.circles.some(r=>r.members.some(m=>m.id===a.id)))return[];
 const choices:Candidate[]=[];
 for(const circle of c.circles){
  const b=visitors.find(v=>v.id===circle.members[0]?.id);
  if(!b||!requested(s,b,now)||busy(s,b.id)||!compatible(a,b)||circle.members.length>=4||circle.members.some(m=>{const v=visitors.find(v=>v.id===m.id);return !v||!available(s,v)||!compatible(a,v);})||!freeSeats(s,circle.spot,visitors,[a.id]).length)continue;
 ...
 for(const b of visitors){
  ...
  for(const spot of SPOTS.filter(p=>p.room===roomAt(a.position))){
   if(c.circles.some(r=>r.spot===spot.id)||freeSeats(s,spot.id,visitors,[a.id,b.id]).length<2)continue;
   choices.push({id:`new:${b.id}:${spot.id}`,to:b.id,spot:spot.id,description:`Nova roda: ${spot.name}; ${ACTIVITIES[spot.activity]}; dois lugares livres`,request:c.requests[b.id].text});
  }
 }
...
export function preparedDecision(request:string,candidates:Candidate[]):Decision{
 const activity=explicitActivity(request)??'stories';const relevant=explicitActivity(request)?candidates.filter(c=>explicitActivity(c.request)===activity):[];
 const selected=relevant.find(c=>c.circle)||relevant.find(c=>SPOTS.find(p=>p.id===c.spot)?.activity===activity)||relevant[0];
...
export function applyCompanyDecision(s:LifeState,id:string,requestId:string,decision:Decision,visitors:Visitor[],now:number){
 ...
 if(!a||!option){request.notice='Ainda não encontrei uma combinação disponível. Seu pedido fica ativo por dez minutos; você pode explorar ou cancelar quando quiser.';return;}
...
export function companyCommand(s:LifeState,cmd:Command,visitors:Visitor[],now:number):string{
 ...
 if(cmd.action==='askCompany'){
  ...
  if((s.cooldown[`company:${v.id}`]??0)>now)return 'Espere um minuto antes de pedir outra sugestão.';
  ...
  c.requests[v.id]={id:cmd.id,text,host:cmd.target==='teo'?'teo':'dora',expires:now+600000,pending:true};s.cooldown[`company:${v.id}`]=now+60000;
 ...
 if(!a||!b||!requested(s,a,now)||!requested(s,b,now)||!compatible(a,b)){c.proposals=c.proposals.filter(r=>r!==p);return 'A disponibilidade mudou. Peça outra sugestão.';}
...
export function tickCompany(s:LifeState,visitors:Visitor[],now:number){
 ...
 c.circles.forEach(r=>{r.members=r.members.filter(m=>{const v=byId.get(m.id);if(v?.seat===m.seat)m.arrived=true;return v&&((v.seat===m.seat)||(!m.arrived&&(m.arriveBy??r.expires)>now))&&v.available!==false&&!s.social.solo[m.id]&&roomAt(v.position)===r.room&&!r.members.some(other=>{const person=byId.get(other.id);return person&&person.id!==v.id&&!compatible(v,person);});});if(r.members.some(m=>byId.get(m.id)?.seat===m.seat))r.expires=now+120000;});
