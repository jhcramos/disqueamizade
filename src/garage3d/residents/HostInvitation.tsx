...
export function HostInvitation({state,visitor,disabled,onAction,onMeet}:{state:LifeState;visitor:string;disabled:boolean;onAction:(action:LifeAction,target:string)=>void;onMeet:(id:string)=>void}){
 ...
 const text=invite.kind==='welcome'?'Quer ajuda para conhecer o pessoal da casa?':ready?`${outgoing?invite.toName:invite.fromName} aceitou conhecer você. ${invite.topic}`:outgoing?`Esperando ${invite.toName} responder. Você pode continuar explorando.`:invite.kind==='ball'?`${invite.fromName} convidou você para jogar a bolinha do Biscoito. Quer pegar a próxima vez?`:`${invite.fromName} quer ${invite.kind==='activity'?'fazer algo junto':'conhecer alguém'}. Quer que eu apresente vocês?`;
