...
type Props={state:LifeState;visitor:string;selected:string|null;mode:Connection;disabled:boolean;message:string;muted:boolean;onMute:()=>void;onSelect:(id:string|null)=>void;onAction:(action:LifeAction,target:string)=>void};
export function ResidentPanel(p:Props){const held=heldItem(p.state,p.visitor),title=p.selected==='bed'?'Caminha do Biscoito':p.selected==='bowl'?'Potinho de água':p.selected==='plant'?'Planta da sala':NAMES[p.selected as ResidentId]??ITEMS[p.selected as keyof typeof ITEMS]?.name;
 return <div className="resident-controls" onPointerDown={e=>e.stopPropagation()}>
  {!p.selected?<button className="resident-open" onClick={()=>p.onSelect('biscoito')}>♧ Moradores e objetos{held?` · ${ITEMS[held.id].name}`:''}</button>:<section className="resident-panel" aria-label="Moradores e objetos">
   ...
   <p className="resident-hint">{p.selected==='biscoito'?'Faça carinho ou traga a bolinha para brincar.':p.selected==='dora'||p.selected==='teo'?'Traga um café, um disco ou venha cumprimentar.':p.selected==='plant'||p.selected==='bowl'?'Pegue o regador para cuidar da casa.':'Toque em uma ação. Seu avatar vai até lá.'}</p>
