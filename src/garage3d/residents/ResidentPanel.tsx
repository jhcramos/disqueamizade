...
import {CompanyRequestForm,CompanySuggestion} from './CompanyPanel';
...
export function ResidentPanel(p:Props){const held=heldItem(p.state,p.visitor),title=p.selected==='bed'?'Caminha do Biscoito':p.selected==='bowl'?'Potinho de água':p.selected==='plant'?'Planta da sala':NAMES[p.selected as ResidentId]??ITEMS[p.selected as keyof typeof ITEMS]?.name;
 ...
 const greeting=!!p.nearbyHost&&!p.selected&&!p.disabled&&!hasInvite;
 return <div className="resident-controls" onPointerDown={e=>e.stopPropagation()}>
  ...
  {!p.selected?(!greeting&&<button className="resident-open" onClick={()=>p.onSelect(p.state.bed.holder===p.visitor?'bed':'biscoito')}>♧ Moradores e objetos{p.state.bed.holder===p.visitor?' · carregando caminha':held?` · ${ITEMS[held.id].name}`:''}</button>):<section className="resident-panel" aria-label="Moradores e objetos">
   ...
   {(p.selected==='dora'||p.selected==='teo')&&<CompanyRequestForm {...p} host={p.selected} disabled={p.disabled||p.mode==='connecting'||p.mode==='reconnecting'}/>}
