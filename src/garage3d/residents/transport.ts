...
export function createResidentTransport(getVisitor:()=>Visitor,onState:(s:LifeState,mode:Connection)=>void,onResult:(text:string)=>void){
 ...
 async function poll(){if(closed||fetching)return;fetching=true;
  ...
  try{const response=await fetch('/api/house-residents',{method:'POST',headers:{'Content-Type':'application/json',...(serverToken?{'X-Resident-Session':serverToken}:{})},body:JSON.stringify({visitor:getVisitor(),command}),signal:AbortSignal.timeout(7000)});
   if(!response.ok){if(response.status===401){serverToken='';serverId='';pending.length=0;mode='reconnecting';emit();onResult('Renovando sua conexão com os moradores. Tente novamente em um instante.');}else if(mode==='connecting'&&(response.status===404||response.status===503)){mode='local';emit();send({type:'hello'});void elect();}else if(mode==='online'){mode='reconnecting';emit();}return;}
 ...
 void poll();
 return {get state(){return state;},get mode(){return mode;},command(action:Command['action'],target:string){const c:Command={id:crypto.randomUUID(),visitor:getVisitor(),action,target};
