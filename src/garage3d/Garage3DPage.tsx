...
export default function Garage3DPage(){
  ...
  function focus(next:View){setView(next);setClose(false);setFirstPerson(false);if(next!=='house')setRoomId(next);controls.current?.focus(next);}
  useEffect(()=>{
    ...
    const avatarBaseY=avatar.position.y;
    ...
    function goToPhone(index:number,id:RoomId){stand();path=routeToPhone(actor.position,id,index);setStatus(path.length?'Indo até o telefone. Ao chegar perto, toque em “Ligar”.':'Não há passagem até este telefone. Experimente outro.');}
    function phone(){const target=nearbyPhone(actor.position);if(!target)return;ringing={index:target.index,room:target.room};ringUntil=performance.now()+6000;setRing(target.index);setStatus('Demonstração da ligação: o telefone toca por alguns segundos. Chamadas reais estão na casa atual.');}
    ...
    controls.current={focus:v=>{first(false);currentView=v;targetZoom=v==='house'?1:roomZoom();if(v!=='house')setLit(lights[v]);},view:v=>{targetZoom=v?3:currentView==='house'?1:roomZoom();},seat:sit,stand,light:id=>{lights[id]=!lights[id];rooms[id].lights(lights[id]);setLit(lights[id]);},phone,approachPhone:id=>goToPhone(3,id),answer,firstPerson:first,lookInside,move:(key,pressed)=>{if(pressed)keys.add(key);else keys.delete(key);},dance:()=>{const next=!dance;stand();dance=next;setDancing(next);if(next){first(false);setStatus('Solta o passinho! Toque no piso para continuar andando.');}}};
    ...
    function onUp(e:PointerEvent){dragging=false;if(Math.hypot(e.clientX-down.x,e.clientY-down.y)>8)return;
      ...
      for(const hit of hits){let root:T.Object3D=hit.object;while(root.parent&&!root.userData.roomId)root=root.parent;const id=root.userData.roomId as RoomId;if(!id)return;
        ...
        if(o.userData.kind==='phone'){const index=o.userData.index as number;const near=nearbyPhone(actor.position);if(near?.index===index&&near.room===id){if(ringing?.index===index&&ringing.room===id)answer();else setStatus('Você está perto do telefone. Toque em “Ligar” para experimentar.');}else goToPhone(index,id);return;}o=o.parent;}
    ...
    function tick(now:number){if(!active)return;const dt=Math.min((now-last)/1000,.05);last=now;let moving=false;
      if(fp&&keys.size){if(sitting)stand();path=[];pendingSeat=null;
        ...
        const next=moveInHouse(actor.position,(Math.sin(yaw)*forward-Math.cos(yaw)*side)*dt*1.7/norm,(Math.cos(yaw)*forward+Math.sin(yaw)*side)*dt*1.7/norm);moving=Math.hypot(next.x-actor.position.x,next.z-actor.position.z)>.001;actor.position.x=next.x;actor.position.z=next.z;actor.rotation.y=yaw;
      }
  ...
  return <main className="garage3d-page">
    ...
    <section className={`garage3d-stage ${firstPerson?'is-first-person':''}`} aria-label="Casa tridimensional integrada">
      ...
      {nearPhone&&<div className="garage3d-phone-prompt"><Phone size={19}/><span>Telefone · {roomNames[nearPhone.room]}<small>Demonstração nesta prévia</small></span><button onClick={()=>ring!==null?controls.current?.answer():controls.current?.phone()}>{ring!==null?'Atender':'Ligar'}</button></div>}
