...
const planLabels:Partial<Record<AreaId,string>>={wc:'WC',bath:'Banho',ensuite:'Banho da suíte',wir1:'Closet',wir3:'Closet',dating:'Quarto 1',games:'Quarto 4',media:'Cinema',porch:'Entrada',study:'Estudo',court:'Pátio',dining:'Jantar'};
type Controls={neighborhood:()=>void;lot:(id:string,walk:boolean)=>void;plan:(enabled:boolean)=>void;area:(id:AreaId)=>void;enteredRoom:(room:RoomId)=>void;focus:(view:View)=>void;view:(close:boolean)=>void;seat:(index:number,room:RoomId)=>void;stand:()=>void;light:(room:RoomId)=>void;phone:()=>void;approachPhone:(room:RoomId)=>void;answer:()=>void;firstPerson:(enabled:boolean)=>void;lookInside:()=>void;move:(key:string,pressed:boolean)=>void;dance:()=>void};
...
export default function Garage3DPage({session}:{session?:LiveHouseSession}={}){
  ...
  const pokerVisitor=useRef<()=>Visitor>(()=>({id:previewVisitor.current,name:'Visitante',position:{...areaById('garage').arrival}}));
  ...
  const lotLabels=useRef(new Map<string,HTMLButtonElement>());
  ...
  useEffect(()=>{
    ...
    const scene=new T.Scene(),camera=new T.OrthographicCamera(),cinemaCamera=new T.OrthographicCamera(-1,1,1,-1,.01,15),eyeCamera=new T.PerspectiveCamera(65,1,.05,180);
    scene.background=new T.Color('#f4ede0');
    const initial=live.current?toWorld(live.current.self.position,live.current.self.room??'garage'):areaById('garage').arrival;
    const cameraFocus=new T.Vector3(initial.x,.65,initial.z);let manualFocus:T.Vector3|null=null,planMode=false;
    camera.position.copy(cameraFocus).add(new T.Vector3(...(planMode?[0,35,.001]:[18,25,26]) as [number,number,number]));camera.lookAt(cameraFocus);camera.near=.1;camera.far=400;
    ...
    const neighborhood=buildNeighborhood(scene);let districtMode=false,districtWide=false,keepDistrictRoom=false,nearestLotId='';
    const cameraOffset=()=>new T.Vector3(...(planMode?[0,35,.001]:districtMode?[72,100,104]:[18,25,26]) as [number,number,number]);
    function leaveDistrict(){districtMode=false;setNeighborhoodView(false);setSelectedLot(null);}
    ...
    const actor=new T.Group();actor.add(avatar);actor.position.set(initial.x,0,initial.z);scene.add(actor);
    ...
    controls.current={neighborhood:()=>{first(false);planMode=false;setPlanView(false);districtMode=true;setNeighborhoodView(true);manualFocus=null;currentView='house';setView('house');setClose(false);targetZoom=el.clientWidth<700?.26:.29;},lot:(id,walk)=>{const lot=lotById(id);if(!lot)return;setSelectedLot(id);if(walk){if(live.current?.frozen)return;stand();path=houseRoute(actor.position,lot.arrival);if(!path.length){setStatus('Não há passagem livre até o terreno.');return;}manualFocus=null;targetZoom=3;setClose(true);setSelectedLot(null);setStatus(`Passeando até o lote ${lot.id} · ${lot.name}.`);}else{manualFocus=new T.Vector3(lot.x,.5,lot.z);targetZoom=1.05;setClose(false);}},plan:enabled=>{leaveDistrict();first(false);planMode=enabled;setPlanView(enabled);manualFocus=null;currentView='house';setView('house');setClose(false);targetZoom=enabled?(el.clientWidth<700?1.08:1.3):1;},area:id=>{leaveDistrict();planMode=false;setPlanView(false);const area=areaById(id);if(live.current?.frozen)return;if(live.current){if(!live.current.onRoom(area.room,toShared(area.arrival,area.room)))return;}else{stand();actor.position.set(area.arrival.x,0,area.arrival.z);}manualFocus=null;currentView=area.room;targetZoom=3;setView(area.room);setRoomId(area.room);setActiveArea(id);setClose(true);},enteredRoom:id=>{if(keepDistrictRoom){keepDistrictRoom=false;currentView='house';setView('house');return;}leaveDistrict();planMode=false;setPlanView(false);manualFocus=null;currentView=id;targetZoom=3;},focus:v=>{leaveDistrict();planMode=false;setPlanView(false);manualFocus=null;first(false);currentView=v;targetZoom=v==='house'?1:roomZoom();if(v!=='house')setLit(lights[v]);},view:v=>{planMode=false;setPlanView(false);manualFocus=null;targetZoom=v?3:districtMode?(el.clientWidth<700?.26:.29):currentView==='house'?1:roomZoom();},seat:sit,stand,light:id=>{const next=live.current?live.current.play.lights?.on===false:!lights[id];if(!play('lights',next))return;lights[id]=next;rooms[id].lights(next);setLit(next);},phone,approachPhone:id=>goToPhone(3,id),answer,firstPerson:first,lookInside,move:(key,pressed)=>{if(pressed)keys.add(key);else keys.delete(key);},dance:()=>{if(live.current?.frozen)return;const next=!dance;if(!play('perform',next))return;stand();dance=next;setDancing(next);if(next){first(false);setStatus('Solta o passinho! Toque no piso para continuar andando.');}}};
    ...
    function moveView(from:{x:number;y:number},to:{x:number;y:number},ratio:number){
      const before=groundAt(from);manualFocus??=cameraFocus.clone();
      zoom=targetZoom=T.MathUtils.clamp(zoom*ratio,.22,4.5);camera.zoom=zoom;camera.updateProjectionMatrix();
      const after=groundAt(to);if(before&&after){manualFocus.x=T.MathUtils.clamp(manualFocus.x+before.x-after.x,NEIGHBORHOOD_BOUNDS.minX,NEIGHBORHOOD_BOUNDS.maxX);manualFocus.z=T.MathUtils.clamp(manualFocus.z+before.z-after.z,NEIGHBORHOOD_BOUNDS.minZ,NEIGHBORHOOD_BOUNDS.maxZ);}
      cameraFocus.copy(manualFocus);camera.position.copy(cameraFocus).add(cameraOffset());camera.lookAt(cameraFocus);camera.updateMatrixWorld();setClose(false);
    }
    function onMove(e:PointerEvent){const delta=gestures.move(e.pointerId,{x:e.clientX,y:e.clientY});if(!delta||live.current?.cinema)return;
      ...
      moveView(delta.from,delta.to,delta.ratio);
    }
    function onWheel(e:WheelEvent){
      if(fp||live.current?.cinema||!e.deltaY)return;
      e.preventDefault();
      // Browsers report wheel distance in pixels, lines or pages.
      const pixels=e.deltaY*(e.deltaMode===1?16:e.deltaMode===2?el.clientHeight:1);
      const point={x:e.clientX,y:e.clientY};
      moveView(point,point,Math.exp(-T.MathUtils.clamp(pixels,-300,300)*.002));
    }
    function onUp(e:PointerEvent){const tap=gestures.up(e.pointerId,{x:e.clientX,y:e.clientY});if(el.hasPointerCapture(e.pointerId))el.releasePointerCapture(e.pointerId);if(!tap||live.current?.frozen||live.current?.cinema)return;
      ...
      const hits=raycaster.intersectObjects([...roomIds.map(id=>rooms[id].root),...residentVisuals.roots.values(),residentFixtures.root,neighborhood.root],true).filter(hit=>{let node:T.Object3D|null=hit.object;while(node){if(!node.visible)return false;node=node.parent;}return true;});
      for(const hit of hits){let parcel:T.Object3D|null=hit.object;while(parcel&&!parcel.userData.lotId)parcel=parcel.parent;if(parcel?.userData.lotId){setSelectedLot(String(parcel.userData.lotId));return;}let outdoor:T.Object3D|null=hit.object;while(outdoor&&!outdoor.userData.neighborhood)outdoor=outdoor.parent;if(outdoor){if(hit.point.y<.065){stand();path=houseRoute(actor.position,hit.point);setStatus(path.length?'Passeando pela vizinhança…':'Escolha um trecho livre da calçada.');}return;}let residentNode:T.Object3D|null=hit.object;while(residentNode&&!residentNode.userData.residentTarget)residentNode=residentNode.parent;if(residentNode?.userData.residentTarget){setResidentTarget(String(residentNode.userData.residentTarget));setLifeMessage('');return;}let root:T.Object3D=hit.object;while(root.parent&&!root.userData.roomId)root=root.parent;const id=root.userData.roomId as RoomId;if(!id)return;
    ...
    function travel(next:Place){const residents=lifeTransport.state.residents.map(r=>r.position);if(residents.some(p=>Math.hypot(next.x-p.x,next.z-p.z)<.43&&Math.hypot(next.x-p.x,next.z-p.z)<Math.hypot(actor.position.x-p.x,actor.position.z-p.z))){const goal=path[path.length-1];if(goal)path=houseRoute(actor.position,goal,residents);return false;}const s=live.current;if(s){
      ...
      const room=roomAt(next);if(room&&room!==s.self.room){keepDistrictRoom=districtMode;roomTransition=true;crossGoal=path[path.length-1]??null;path=[];if(!s.onRoom(room,toShared(next,room))){keys.clear();crossGoal=null;}return false;}
    ...
    el.addEventListener('wheel',onWheel,{passive:false});el.addEventListener('pointerdown',onDown);el.addEventListener('pointermove',onMove);el.addEventListener('pointerup',onUp);el.addEventListener('pointercancel',release);el.addEventListener('lostpointercapture',endDrag);
    ...
    const requestedNeighborhood=new URLSearchParams(location.search).has('bairro');if(requestedNeighborhood){controls.current.neighborhood();}
    function tick(now:number){if(!active)return;if(live.current?.cinema&&now-last<1000/30){frame=requestAnimationFrame(tick);return;}const dt=Math.min((now-last)/1000,.05);last=now;let moving=false;const walkSpeed=areaAt(actor.position)?1.7:2.6;roomTransition=false;
      ...
      if(fp&&keys.size&&!state?.frozen){if(sitting)stand();path=[];pendingSeat=null;
        ...
        const next=moveInHouse(actor.position,(Math.sin(yaw)*forward-Math.cos(yaw)*side)*dt*walkSpeed/norm,(Math.cos(yaw)*forward+Math.sin(yaw)*side)*dt*walkSpeed/norm);moving=Math.hypot(next.x-actor.position.x,next.z-actor.position.z)>.001&&travel(next);actor.rotation.y=yaw;
      }
      if(path.length){const p=path[0],dx=p.x-actor.position.x,dz=p.z-actor.position.z,d=Math.hypot(dx,dz),step=Math.min(d,dt*walkSpeed);
      ...
      const focus=manualFocus??(targetZoom===3?new T.Vector3(actor.position.x,.65,actor.position.z):currentView==='house'?new T.Vector3(-.15,.5,-1):new T.Vector3(roomOffsets[currentView].x,.5,roomOffsets[currentView].z));
      ...
      const wide=districtMode&&!fp&&zoom<.7;if(wide!==districtWide){districtWide=wide;setNeighborhoodWide(wide);}
      for(const lot of NEIGHBORHOOD_LOTS){const button=lotLabels.current.get(lot.id);if(!button)continue;const p=new T.Vector3(lot.x,.1,lot.z).project(camera);button.style.left=`${(p.x+1)*50}%`;button.style.top=`${(1-p.y)*50}%`;button.hidden=fp||!districtMode||zoom>.65||Math.abs(p.x)>.93||Math.abs(p.y)>.9||p.z>1||p.z< -1;}
      const closestLot=NEIGHBORHOOD_LOTS.find(l=>Math.hypot(l.sign.x-actor.position.x,l.sign.z-actor.position.z)<3)?.id??'';if(closestLot!==nearestLotId){nearestLotId=closestLot;setNearLot(closestLot||null);}
      ...
      rooms.living.cutaway(!fp&&!manualFocus&&targetZoom===3&&areaAt(actor.position)?.id==='pool'&&!state?.cinema);
      ...
      el.dataset.nearLot=nearestLotId;
  ...
  return <div className={`garage3d-page${session?' is-live-house':''}${planView?' is-plan-view':''}${neighborhoodView?' is-neighborhood':''}${neighborhoodWide?' is-neighborhood-wide':''}${selectedLot?' has-selected-lot':''}${selectedSeatMarker!==null?' has-selected-seat':''}${session?.cinema?' is-cinema':''}`}>
    ...
    <nav className="garage3d-rooms" aria-label="Enquadramento da casa">
      ...
      <button aria-pressed={view==='house'&&!close&&!planView&&!neighborhoodView} onClick={()=>focus('house')}>Casa inteira</button>
      <button aria-pressed={neighborhoodView} onClick={()=>controls.current?.neighborhood()}>Vizinhança</button>
      <button aria-pressed={planView} onClick={()=>controls.current?.plan(!planView)}>Vista da planta</button>
    ...
    <section className={`garage3d-stage ${firstPerson?'is-first-person':''}`} aria-label="Casa tridimensional integrada" onPointerDown={e=>{if(e.target instanceof Element&&!e.target.closest('.garage3d-seat-anchor'))setSelectedSeatMarker(null);}}>
      ...
      <div ref={host} className="garage3d-canvas" aria-label="Toque para andar. Arraste para mover a vista. Use a roda do mouse ou dois dedos para dar zoom."/>
      ...
      {!firstPerson&&<span className="house-gesture-hint"><span className="house-gesture-mouse">Clique para andar · Arraste a vista · Scroll para zoom</span><span className="house-gesture-touch">Toque para andar · Arraste a vista · Pinça para zoom</span></span>}
