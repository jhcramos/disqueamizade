...
export default function Garage3DPage({session}:{session?:LiveHouseSession}={}){
  ...
  useEffect(()=>{
    ...
    function onMove(e:PointerEvent){const delta=gestures.move(e.pointerId,{x:e.clientX,y:e.clientY});if(!delta||live.current?.cinema)return;
      ...
      zoom=targetZoom=T.MathUtils.clamp(zoom*delta.ratio,.22,4.5);camera.zoom=zoom;camera.updateProjectionMatrix();
      const after=groundAt(delta.to);if(before&&after){manualFocus.x=T.MathUtils.clamp(manualFocus.x+before.x-after.x,NEIGHBORHOOD_BOUNDS.minX,NEIGHBORHOOD_BOUNDS.maxX);manualFocus.z=T.MathUtils.clamp(manualFocus.z+before.z-after.z,NEIGHBORHOOD_BOUNDS.minZ,NEIGHBORHOOD_BOUNDS.maxZ);}
