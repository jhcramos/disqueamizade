...
type Controls={plan:(enabled:boolean)=>void;area:(id:AreaId)=>void;enteredRoom:(room:RoomId)=>void;focus:(view:View)=>void;view:(close:boolean)=>void;seat:(index:number,room:RoomId)=>void;stand:()=>void;light:(room:RoomId)=>void;phone:()=>void;approachPhone:(room:RoomId)=>void;answer:()=>void;firstPerson:(enabled:boolean)=>void;lookInside:()=>void;move:(key:string,pressed:boolean)=>void;dance:()=>void};
...
export default function Garage3DPage({session}:{session?:LiveHouseSession}={}){
  ...
  useEffect(()=>{
    ...
    camera.position.copy(cameraFocus).add(new T.Vector3(...(planMode?[0,35,.001]:[18,25,26]) as [number,number,number]));camera.lookAt(cameraFocus);camera.near=.1;camera.far=100;
    ...
    function onMove(e:PointerEvent){const delta=gestures.move(e.pointerId,{x:e.clientX,y:e.clientY});if(!delta||live.current?.cinema)return;
      ...
      cameraFocus.copy(manualFocus);camera.position.copy(cameraFocus).add(new T.Vector3(...(planMode?[0,35,.001]:[18,25,26]) as [number,number,number]));camera.lookAt(cameraFocus);camera.updateMatrixWorld();setClose(false);
    }
    ...
    function tick(now:number){if(!active)return;if(live.current?.cinema&&now-last<1000/30){frame=requestAnimationFrame(tick);return;}const dt=Math.min((now-last)/1000,.05);last=now;let moving=false;roomTransition=false;
      ...
      cameraFocus.lerp(focus,reduced?1:Math.min(1,dt*4));camera.position.copy(cameraFocus).add(new T.Vector3(...(planMode?[0,35,.001]:[18,25,26]) as [number,number,number]));camera.lookAt(cameraFocus);camera.updateProjectionMatrix();
  ...
  return <div className={`garage3d-page${session?' is-live-house':''}${planView?' is-plan-view':''}${session?.cinema?' is-cinema':''}`}>
    ...
    <nav className="garage3d-rooms" aria-label="Enquadramento da casa">
      ...
      <button aria-pressed={view==='house'&&!close&&!planView} onClick={()=>focus('house')}>Casa inteira</button>
